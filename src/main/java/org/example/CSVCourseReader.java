...
public class CSVCourseReader {
    ...
    public static List<Course> readCoursesFromCSV(String filePath) {
        ...
        try (InputStream csvFileStream = CSVCourseReader.class.getResourceAsStream("/" + filePath);
            ...
            for (String[] record : records) {
                ...
                // Check the color field and assign a default color if it's empty
                String color = record[18].trim();
                color = assignDefaultColor(color); // Pass the current color (or empty) for default color assignment
    ...
    // Method to filter courses by color
    public static List<Course> filterCoursesByColor(List<Course> courses, String color) {
        return courses.stream()
                .filter(course -> course.getColor().equalsIgnoreCase(color)) // Case-insensitive comparison
                .collect(Collectors.toList());
    }

    /**
     * Assigns a default color to each course if the color is empty.
     * If a color is provided, it will be used as is.
     *
     * @param color the current color of the course.
     * @return the final color for the course (either provided or assigned).
     */
    private static String assignDefaultColor(String color) {
        // If the color is empty, assign a random color
        if (color == null || color.trim().isEmpty()) {
            String[] colors = { "Red", "Blue", "Green", "Yellow", "Orange", "Purple" };
            int index = (int) (Math.random() * colors.length);
            return colors[index]; // Return a random color
        }
        return color; // Return the provided color if it's not empty
    }
}